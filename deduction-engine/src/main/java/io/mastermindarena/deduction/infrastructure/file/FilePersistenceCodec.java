...
public final class FilePersistenceCodec {
    ...
    public static String serializeMatchRuntimeState(MatchRuntimeState state) {
        ...
        return String.join("\t",
                ...
                state.actorOrder().stream().map(id -> Integer.toString(state.scores().getOrDefault(id, 0))).collect(Collectors.joining(",")),
                encode(state.winnerId() == null ? "" : state.winnerId()),
                Integer.toString(state.gameNumber())
        );
    }
    ...
    public static MatchRuntimeState deserializeMatchRuntimeState(String line) {
        ...
        if (fields.length == 15) {
            String[] counts = fields[12].split(",", -1);
            for (int i = 0; i < actorOrder.size() && i < counts.length; i++) {
                int count = Integer.parseInt(counts[i]);
                if (count > 0) scores.put(actorOrder.get(i), count);
            }
        }
        String winnerId = fields.length == 15 ? decode(fields[13]) : "";
        int gameNumber = fields.length == 15 ? Integer.parseInt(fields[14]) : 1;
        return new MatchRuntimeState(
                ...
                legacyOutcome,
                scores,
                winnerId.isBlank() ? null : winnerId,
                gameNumber
        );
    }
